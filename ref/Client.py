...
def create_new_window():
    ...
    new_window_label = tk.Label(new_window, text="Enter your message:")
    new_window_label.pack()

    new_window_entry = tk.Entry(new_window, font=("Helvetica", 16))
    new_window_entry.pack()

    new_window_button = tk.Button(new_window, text="Send", command=lambda: send_new_msg(new_window_entry))
    new_window_button.pack()

    new_window_quit_button = tk.Button(new_window, text="Quit", command=new_window.destroy)
    new_window_quit_button.pack()

# SFTP 서버 정보
hostname = "192.0.0.1" # IP
port = 22
username = "username"
password = "password"
...
def send_new_msg(entry):
    msg = entry.get()
    entry.delete(0, tk.END)
    filename=os.path.basename(msg)
    sftp.put(msg, "~/"+filename)
    sftp.get(msg,"~/"+filename)


def start_client():
    root = tk.Tk()
    chat_window = ChatWindow(root)
    new_window_button = tk.Button(root, text="Open New Window", command=create_new_window)
    new_window_button.pack()
    root.mainloop()
